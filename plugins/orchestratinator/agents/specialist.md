...
- You may use judgment on implementation details that stay inside the task's Files (internal helper structure, how to debug). Anything visible outside those files, or anything that differs from the sources, Decisions, or Steps, is a `GAP`: report it instead of deciding.
