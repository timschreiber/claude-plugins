...
.DESCRIPTION
    ...
    ported here. CommandSegmentation.Tests.ps1 carries the same vectors --
...
function Get-SegmentEdit {
    <#
    .SYNOPSIS
        Insertion points for one span, recursing into subshells.
    ...
    if ($Depth -lt 4 -and $text.StartsWith('(') -and $text.EndsWith(')')) {
        ...
        foreach ($sub in (Split-CommandSegment -Command $inner)) {
            ...
            foreach ($i in (Get-SegmentEdit -Command $Command -Span $shifted -Prefixes $Prefixes -Depth ($Depth + 1))) {
                $out.Add($i)
            }
        }
        return $out
    ...
    if (Test-SegmentPrefix -Command $Command -Span $Span -Prefixes $Prefixes) {
        return @(Get-InsertionPoint -Command $Command -Span $Span)
    }
    return @()
}
...
function Add-CommandFlag {
    <#
    .SYNOPSIS
        Append flags to every top-level segment invoking a target command.

    .EXAMPLE
        Add-CommandFlag -Command 'cd src && dotnet build' `
                        -Prefixes @('dotnet build') -Flags '-nologo -tl:off'
    ...
    $edits = [System.Collections.Generic.List[int]]::new()
    foreach ($span in (Split-CommandSegment -Command $Command)) {
        foreach ($idx in (Get-SegmentEdit -Command $Command -Span $span -Prefixes $Prefixes -Depth 0)) {
            $edits.Add($idx)
        }
    }

    $result = $Command
    foreach ($idx in ($edits | Sort-Object -Descending)) {
        $result = $result.Substring(0, $idx) + ' ' + $Flags + $result.Substring($idx)
    }
    return $result
}
